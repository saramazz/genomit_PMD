...
from utilities import (
    ...
    find_common_important_vars, 
    count_nan_per_column, 
    calculate_total, 
    remove_hp_prefix, 
    add_top5_symptoms_columns, 
    update_encephalopathy_column, 
    add_abnormalities_cols, 
    order_columns_alphabetically, 
    check_multiple_patients, 
    convert_to_numerical
)
from processing import process_form, process_datetime_column
from plotting import plot_phenotype_distribution, plot_missing_values, plot_missing_values_reduced, plot_missing_values_diff
...
def plot_missing_values(df, hospital_name, saved_result_path):
    ...
    missing_data = pd.DataFrame({'Missing Values': missing_values, 'Missing Percentage': missing_percentages})
...
def plot_missing_values_reduced(df, hospital_name):
    ...
    excluded_columns = [col for col in df.columns if 'psterm__decod_' in col or 'pimgtype_' in col or 'symp' in col]
...
def plot_missing_values_diff(df, hospital_name, saved_result_path): #difference between IT presence or absence
    ...
    missing_percentage_df = missing_percentage_df.sort_values(by='Total Missing Percentage', ascending=False)

    # Plotting
    plt.figure(figsize=(100, 8))  # Increased width
    ax = missing_percentage_df.plot(kind='bar', width=0.8, edgecolor='k')#, colormap='viridis')
...
def plot_histogram_visits_per_patient(df, hospital_name, saved_result_path):
    visit_count = df.groupby('subjid')['visdat'].nunique().reset_index(name='num_visits')
    unique_values = visit_count['num_visits'].unique()
    print("Unique values in 'num_visits':", unique_values)
    n, bins, patches = plt.hist(visit_count['num_visits'], bins=range(0, 10), align='left', color='#0504aa', alpha=0.7)
    plt.grid(axis='y', linestyle='--', linewidth=0.7, alpha=0.7)
    for i in range(len(patches)):
        plt.text(patches[i].get_x() + patches[i].get_width() / 2.0, patches[i].get_height(), str(int(n[i])), ha='center', va='bottom')
...
def plot_gendna_distribution(df):
    ...
    class_counts.plot.pie(autopct=lambda p: f'{p:.1f}%', figsize=(15, 8), labels=class_counts.index, legend=True, colors=colors)
    ...
    plt.savefig(os.path.join(saved_result_path_classification, my_file), bbox_inches="tight")
    plt.close()
...
def precision_recall_plot(gt, pp, title, file_name, global_path):
    ...
    plt.savefig(os.path.join(saved_result_path_classification, my_file), format="png", bbox_inches="tight")
...
def confusion_matrix_plot(conf_mat, class_labels, file_name):
    ...
    sns_plot = sns.heatmap(conf_mat, annot=labels, fmt='', cmap='Blues', annot_kws={"size": 18})
